-- Start "正解は...？" and the drumroll together, then reveal the answer on the
-- drumroll source's "じゃん!" hit (~2.54s). Previously the server held for 4s and
-- the client delayed audio start, so the prompt appeared before the BGM.
